...
pub fn get_target_dir(path: &Path) -> Result<PathBuf, Box<dyn std::error::Error>> {
    ...
    if !resolved.exists() {
        return Err(format!("{}: {:?}", gettext("Directory does not exist"), resolved).into());
    }
    let pkgbuild_path = resolved.join("PKGBUILD");
    if !pkgbuild_path.exists() {
        return Err(format!("{}: {:?}", gettext("No PKGBUILD found in directory"), resolved).into());
    }
