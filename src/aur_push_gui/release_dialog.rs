...
fn load_win_size(key: &str, dw: i32, dh: i32) -> (i32, i32) {
    (|| -> Option<(i32, i32)> {
        ...
        let o = val.get(key)?;
        Some((o.get("width")?.as_i64()? as i32, o.get("height")?.as_i64()? as i32))
    })()
    .unwrap_or((dw, dh))
}
...
fn save_win_size(key: &str, w: i32, h: i32) {
    ...
    .unwrap_or_default();
    obj.insert(key.to_string(), serde_json::json!({"width": w, "height": h}));
...
impl ProgressPanel {
    fn new(run_label: &str) -> Self {
        ...
        ProgressPanel { root, caption, bar, steps_group, error_box, error_view, status_page, run_btn, back_btn }
    }
...
impl ReleaseWindow {
    pub fn new(app: &adw::Application, target: String) -> ApplicationWindow {
        ...
        let make_nav_btn = |icon: &str, label_text: &str, page: &str, stack: Stack| {
            ...
            btn.connect_clicked(move |_| { stack.set_visible_child_name(&page_name); });
            btn
        };

        let push_nav = make_nav_btn(
            "send-symbolic", "Push", "push", content_stack.clone()
        );
        let tags_nav = make_nav_btn(
            "bookmark-new-symbolic", "Tags", "tags", content_stack.clone()
        );
        ...
        let push_msg = adw::EntryRow::builder()
            .title("Commit message").show_apply_button(false).build();
        push_group.add(&push_msg);

        // Branch picker
        let push_branch: Rc<adw::EntryRow> = Rc::new(
            adw::EntryRow::builder().title("Branch").show_apply_button(false).build()
        );
        {
            ...
            for b in &branches {
                ...
                let pb = Button::builder()
                    ...
                    .build();
                let bn = b.clone(); let ec = push_branch.clone();
                pb.connect_clicked(move |_| { ec.set_text(&bn); });
                pop_box.append(&pb);
            }
            let pop = Popover::builder().child(&pop_box).build();
            let pick = Button::builder()
                .icon_name("vcs-branch-symbolic").tooltip_text("Choose branch")
                .valign(Align::Center).css_classes(vec!["flat".to_string()])
                .build();
            pop.set_parent(&pick);
            pick.connect_clicked(clone!(#[strong] pop, move |_| { pop.popup(); }));
        ...
        if !remote_url.is_empty() {
            let rem_row = adw::ActionRow::builder()
                .title("Remote").subtitle(&remote_url).activatable(false).build();
            let rem_icon = gtk::Image::builder()
                .icon_name("network-server-symbolic").pixel_size(16)
                .valign(Align::Center).css_classes(vec!["dim-label".to_string()])
                .build();
        ...
        let push_prog_scroll = ScrolledWindow::builder()
            ...
            .vscrollbar_policy(PolicyType::Automatic)
            .vexpand(true).build();
        let push_panel = ProgressPanel::new(
            &format!("Commit & Push → {}", platform.label())
        );
        push_panel.caption.set_label("Step 2 of 2 — Sending changes to remote");
        let push_steps = vec![
        ...
        for (_, s) in &push_steps { push_panel.steps_group.add(&s.row); }
        ...
        let tag_name_row = adw::EntryRow::builder()
            .title("Tag name  (e.g. v1.0.0)").show_apply_button(false).build();
        new_tag_group.add(&tag_name_row);

        let tag_msg_row = adw::EntryRow::builder()
            .title("Tag message  (e.g. Version 1.0.0)").show_apply_button(false).build();
        ...
        tags_panel.caption.set_label("Step 2 of 2 — Creating and pushing tag");
        let tag_steps = vec![
            ("git-tag",       StepRow::new("git tag -a")),
            ("git-push-tag",  StepRow::new("git push origin <tag>")),
        ];
        for (_, s) in &tag_steps { tags_panel.steps_group.add(&s.row); }
        ...
        if platform.supports_releases() {
            ...
            rel_branch_entry.set_text(&detect_branch(&target));
            {
                ...
                for b in &branches {
                    ...
                    let pb = Button::builder()
                        ...
                        .css_classes(if is_c {
                            vec!["branch-item".to_string(), "branch-item-current".to_string()]
                        } else { vec!["branch-item".to_string()] })
                        .build();
                    let bn = b.clone(); let ec = rel_branch_entry.clone();
                    pb.connect_clicked(move |_| { ec.set_text(&bn); });
                    pop_box.append(&pb);
                }
                let pop = Popover::builder().child(&pop_box).build();
            ...
            add_file_btn.connect_clicked(clone!(
                ...
                move |_| {
                    ...
                    chooser.open(Some(&win), None::<&gtk::gio::Cancellable>,
                        clone!(
                            #[strong] attachments, #[strong] attach_list_box,
                            move |result| {
                                if let Ok(file) = result {
                                    if let Some(path) = file.path() {
                                        ...
                                        let flbl = Label::builder()
                                            .label(&fname).hexpand(true).halign(Align::Start)
            ...
            for (_, s) in &rel_steps { rel_panel.steps_group.add(&s.row); }
            ...
            rel_run_btn2.connect_clicked(clone!(
                ...
                move |_| {
                    ...
                    *rel_done_st.borrow_mut() = 0;
                    ...
                    let notes  = buf.text(&buf.start_iter(), &buf.end_iter(), false).to_string();
    ...
    match msg {
        ...
        Msg::Step { key, state, detail } => {
            for (k, step) in steps.iter() {
                if *k == key {
                    match state {
                        StepState::Start => { step.set_running(); bar.pulse(); }
                        StepState::Ok => {
                            step.set_ok();
...
macro_rules! step {
    (start $tx:expr, $k:expr) => {
        let _ = $tx.send_blocking(Msg::Step { key: $k.to_string(), state: StepState::Start, detail: String::new() });
    };
    (ok $tx:expr, $k:expr) => {
        let _ = $tx.send_blocking(Msg::Step { key: $k.to_string(), state: StepState::Ok, detail: String::new() });
    };
    (err $tx:expr, $k:expr, $d:expr) => {
        let _ = $tx.send_blocking(Msg::Step { key: $k.to_string(), state: StepState::Error, detail: $d.to_string() });
    };
}
...
fn run_push_worker(
    ...
    let cm = message.as_deref().filter(|s| !s.is_empty()).unwrap_or("Update");
...
fn run_tag_worker(
    ...
    let tag_name = if name.starts_with('v') { name.clone() } else { format!("v{name}") };
    let tag_msg  = if msg.is_empty() { format!("Version {}", name.trim_start_matches('v')) } else { msg };
...
fn run_release_worker(
    ...
    let tag_name  = if tag.starts_with('v') { tag.clone() } else { format!("v{tag}") };
    let rel_title = if title.is_empty() { tag_name.clone() } else { title };
    ...
    let ok = match platform {
        Platform::GitHub => {
            let mut args: Vec<String> = vec![
                ...
                "--notes".into(), notes.clone(),
                "--target".into(), branch.clone(),
            ];
            for f in &attachments { args.push(f.clone()); }
        ...
        Platform::GitLab => {
            let mut args: Vec<String> = vec![
                ...
                "--notes".into(), notes.clone(),
                "--ref".into(), branch.clone(),
            ];
            for f in &attachments { args.push("--assets-links".into()); args.push(f.clone()); }
            let args_ref: Vec<&str> = args.iter().map(String::as_str).collect();
...
fn run_cli(cmd: &str, args: &[&str], cwd: &str, tx: &async_channel::Sender<Msg>) -> bool {
        ...
        .stdout(Stdio::piped()).stderr(Stdio::piped()).spawn()
    {
        ...
        Err(e) => {
            let _ = tx.send_blocking(Msg::Log(
                format!("{cmd} not found or failed to start: {e}\nMake sure '{cmd}' is installed.")
            ));
            return false;
        }
    };
