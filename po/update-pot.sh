...
#
#  Step 3 — Bash extraction + POTFILES.in regeneration
#    Extracts the English msgid (argument 1) and its temporary legacy key
#    (argument 2) directly from _i18n.
#    Rewrites po/POTFILES.in from the discovered file list.
#    Do not edit this file manually — it is always overwritten.
#
#  Step 4 — .pot merge
#    Combines rust.pot + python.pot + bash.pot into a single merged.pot via
#    msgcat --use-first.
#
#  Step 5 — Header fixup
...
#
#  Step 6 — Bidirectional LINGUAS <-> .po sync
...
echo "[1/6] Discovering Rust files (.rs with gettext())..."
...
echo "[2/6] Discovering Python files (.py with _())..."
mapfile -t PY_FILES < <(
    {
...
# ── 3. Extract Bash _getmsg() strings and update POTFILES.in ─────────────────
echo "[3/6] Extracting Bash _getmsg() strings and updating POTFILES.in..."
...
echo "   → POTFILES.in updated with $((${#RUST_FILES[@]} + ${#PY_FILES[@]} + 1)) entries"

# ── 4. Merge all sources ──────────────────────────────────────────────────────
echo "[4/6] Merging extracted Rust, Python and Bash .pot files..."
MERGE=()
[[ -f "$TMP/rust.pot" ]]   && MERGE+=("$TMP/rust.pot")
[[ -f "$TMP/python.pot" ]] && MERGE+=("$TMP/python.pot")
[[ -f "$TMP/bash.pot" ]]   && MERGE+=("$TMP/bash.pot")
[[ -f "$TMP/bash-legacy.pot" ]] && MERGE+=("$TMP/bash-legacy.pot")
...
# ── 5. Fix header and write final .pot ────────────────────────────────────────
echo "[5/6] Writing $OUT..."
